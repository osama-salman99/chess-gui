...
class PositionValidatorTest {
	...
	@Test
	void givenCastlingDestinationAttackedWhenValidateCastlingThenThrowKingInCheck() {
		Piece king = new King(PieceColor.WHITE, pos(File.E, Rank.FIRST));
		Piece rook = new Rook(PieceColor.WHITE, pos(File.H, Rank.FIRST));
		// Black rook controls g1 (castling destination for kingside)
		Piece blackRook = new Rook(PieceColor.BLACK, pos(File.G, Rank.EIGHTH));
		Piece bk = new King(PieceColor.BLACK, pos(File.A, Rank.EIGHTH));
		PositionValidator v = validatorWith(List.of(king, rook, blackRook, bk), PieceColor.WHITE);

		assertThrows(KingInCheckException.class,
				() -> v.validateMove(king, pos(File.G, Rank.FIRST)));
	}
